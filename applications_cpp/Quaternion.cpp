...
void Quaternion::MadgwickAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz)
{
	...
	float &q0 = w, &q1 = x, &q2 = y, &q3 = z;	// quaternion of sensor frame relative to auxiliary frame
	...
	q0 += qDot1 * sampleInterval;
	q1 += qDot2 * sampleInterval;
	q2 += qDot3 * sampleInterval;
	q3 += qDot4 * sampleInterval;
...
void Quaternion::MadgwickAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay, float az)
{
	float recipNorm;
	float s0, s1, s2, s3;
	float qDot1, qDot2, qDot3, qDot4;
